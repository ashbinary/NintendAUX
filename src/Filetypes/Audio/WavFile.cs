...
public struct WavFile
{
    ...
    public struct WavFormat
    {
        ...
        public ushort
            BlockAlign; // (BitsPerSample * Channels) / 8.1 - 8 bit mono2 - 8 bit stereo/16 bit mono4 - 16 bit stereo
    ...
    public static byte[] Write(WavFile wavData)
    {
        ...
        wavData.Format.ByteRate =
            (uint)(wavData.Format.SampleRate * wavData.Format.NumChannels * (wavData.Format.BitsPerSample / 8));
        wavData.Format.BlockAlign = (ushort)(wavData.Format.NumChannels * (wavData.Format.BitsPerSample / 8));

        var samplesPerChannel = (uint)wavData.AudioData[0].Data.Length;
        var dataSize = samplesPerChannel * wavData.Format.NumChannels * sizeof(short);
